-- ============================================
-- VENSER Database Schema - Setup Completo
...
-- Policies para DAILY_MISSIONS
DROP POLICY IF EXISTS "Anyone can view active missions" ON public.daily_missions;
...
DROP POLICY IF EXISTS "Users can insert their own mission completions" ON public.mission_completions;
...
DROP POLICY IF EXISTS "Users can view their own productivity entries" ON public.productivity_entries;
CREATE POLICY "Users can view their own productivity entries"
  ON public.productivity_entries FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own productivity entries" ON public.productivity_entries;
CREATE POLICY "Users can insert their own productivity entries"
  ON public.productivity_entries FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own productivity entries" ON public.productivity_entries;
